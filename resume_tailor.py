...
def convert_json_to_markdown(tailored_resume_json):
    ...
    for key, value in contact.items():
        if key != "location":  # Skip location as it's displayed separately
            if key.lower() == "whatsapp":
                if "wa.me/" in value:
                    phone_number = value.split("wa.me/")[1]
                    contact_links.append(f'<a href="{value}" target="_blank" style="margin: 0 0.5em; color: #0366d6; text-decoration: none; font-size: 1.2em;">+{phone_number}</a>')
                else:
                    contact_links.append(f'<a href="{value}" target="_blank" style="margin: 0 0.5em; color: #0366d6; text-decoration: none; font-size: 1.2em;">{value}</a>')
            else:
                contact_links.append(f'<a href="{value}" target="_blank" style="margin: 0 0.5em; color: #0366d6; text-decoration: none; font-size: 1.2em;">{value}</a>')
