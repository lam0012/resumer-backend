...
if ENVIRONMENT == "production":
    allowed_origins = [
        ...
        "https://cv1234.vercel.app",  # Add your Vercel frontend URL
...
async def download_resume(filename: str, mode: Optional[str] = None):
    ...
    try:
        ...
        if not file_path.exists():
            ...
            raise HTTPException(
                status_code=404, 
                detail=f"Resume not found. Available files: {[f.name for f in available_files]}"
            )
...
async def debug_files(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        return {
            ...
            "files": [{"name": f.name, "size": f.stat().st_size if f.exists() else 0} for f in output_files],
